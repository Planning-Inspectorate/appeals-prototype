module.exports = function (router) {
  ...
  var base = "/enforcement/"+v+"/";
    ...
    ******************************/
    ...
    router.post(base+'contact-details/who-are-you', function (req, res) {
      ...
      if (user === 'appellant') {
        res.redirect('/enforcement/v3/contact-details/contact-details')
      } else {
        res.redirect('/enforcement/v3/contact-details/who-was-served')
      }
    })
  ...
  router.post('/sitevisible', function (req, res) {
    ...
    if (visibility === 'yes') {
      ...
      res.redirect('/enforcement/v3/appeal-site/postcode-address')
    }
  })
  ...
  router.post(base+'appeal-site/siteinterest', function (req, res) {
    ...
    // route depending on value
    if (interest === 'none') {
      res.redirect('/enforcement/v3/appeal-site/owner-consent')
    } else {
      req.session.data["enforcement-taskliststatus-appealsite"] = "Complete";
      res.redirect('/enforcement/v3/task-list?enforcement-taskliststatus-appealsite=Complete')
    }
  })
  ...
  router.post(base+'appeal-site/owner-consent', function (req, res) {
    ...
    // route depending on value
    if (interest === 'Yes') {
      res.redirect('/enforcement/v3/appeal-site/site-occupation')
    } else {
      res.redirect('/enforcement/v3/appeal-site/your-interest')
    }
  })
  ...
  router.post(base+'grounds/grounds', function (req, res) {
    ...
    // res.redirect('/enforcement/v3/grounds/upload-reasons')
    ...
    if (groundA) {
      ...
      res.redirect('/enforcement/v3/grounds/groundA')
    } else if (groundB) {
      res.redirect('/enforcement/v3/grounds/groundB')
    } else if (groundC) {
      res.redirect('/enforcement/v3/grounds/groundC')
    } else if (groundD) {
      res.redirect('/enforcement/v3/grounds/groundD')
    } else if (groundE) {
      res.redirect('/enforcement/v3/grounds/groundE')
    } else if (groundF) {
      res.redirect('/enforcement/v3/grounds/groundF')
    } else if (groundG) {
      res.redirect('/enforcement/v3/grounds/groundG')
    } else {
      res.redirect('/enforcement/v3/grounds/application')
    }
  })
  ...
  router.post('/applicationmade-v3', function (req, res) {
    ...
    if (application === 'Yes') {
      res.redirect('/enforcement/v3/grounds/application-date')
    } else {

      if ( groundA === 'Yes') {
        res.redirect('/enforcement/v3/grounds/fees')
      } else {
  ...
  router.post('/applicationdecision-v3', function (req, res) {
    ...
    if (decisionreceived === 'Yes') {
      ...
      res.redirect('/enforcement/v3/grounds/application-decision-date')
  ...
  router.post('/feespaid-v3', function (req, res) {
    ...
    if (feespaid === 'Yes') {
      ...
      res.redirect('/enforcement/v3/grounds/fees-exempt')
    }
  })
  ...
  router.post(base+'procedure/procedure', function (req, res) {
    ...
    if (how === 'Written representations') {
      ...
      res.redirect('/enforcement/v3/procedure/hearing')
    } else {
      res.redirect('/enforcement/v3/procedure/inquiry')
    }
  })
  ...
  router.post(base+'upload/planning-obligation-status', function (req, res) {
    ...
    if (obligationstatus === 'Finalised and ready to submit') {
      res.redirect('/enforcement/v3/upload/obligation-ready')
    } else if (obligationstatus === 'In draft') {
      res.redirect('/enforcement/v3/upload/obligation-draft')
    } else {
      res.redirect('/enforcement/v3/upload/obligation-not-started')
    }
  })
  ...
  router.post(base+'upload/supporting-documents', function (req, res) {
    ...
    if (supportingdocuments === 'Yes') {
      res.redirect('/enforcement/v3/upload/additional-documents')
    } else {
      req.session.data["enforcement-taskliststatus-appealdocuments"] = "Complete";
      res.redirect('/enforcement/v3/task-list')
    }
  ...
  router.get(base+'skip/appeal-start', function (req, res) {
    ...
    res.redirect('/enforcement/v3/before-you-continue')
  })
