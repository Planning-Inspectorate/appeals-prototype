...
router.get('/task-list', function(req, res, next){
  ...
  if (!isComplete('env-impact-completed') && hasValue(data['schedule'])) {
    data['env-impact-started'] = 'true'
  }
...
router.post('/constraints/:page', function (req, res, next) {
  ...
  if (
    req.session.data['correct-type-of-appeal-check-complete']
...
router.post('/constraints/correct-type-of-appeal-check', function (req, res) {
...
router.post('/og-evidence/:page', function (req, res, next) {
  ...
  // if all 6 questions are complete
...
router.post('/og-evidence/plans-and-drawings-check', function (req, res) {
   if (req.session.data['plans-and-drawings-check'] == 'Yes') {
    res.redirect('plans-and-drawings-upload');
    req.session.data['og-evidence-in-progress'] = 'true'
  } else {
    res.redirect('other-documents-check')
  }
})
...
router.post('/og-evidence/other-documents-check', function (req, res) {
  if (req.session.data['other-documents-check'] == 'Yes') {
    res.redirect('other-documents-upload');
  } else {
    res.redirect('list-of-documents')
  }
})

router.post('/og-evidence/other-documents-upload', function (req, res) {
  res.redirect('list-of-documents')
})
...
router.post('/env-impact/:page', function (req, res, next) {
  ...
  // Section is complete once the applicant has either uploaded an
  // environmental statement, or confirmed one isn't being submitted
  if (
    req.session.data['env-statement-upload']
