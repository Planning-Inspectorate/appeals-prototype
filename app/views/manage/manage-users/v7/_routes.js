...
const renderManageUsersIndex = (req, res) => {
  normalizeUsers(req)
...
router.get('/', function (req, res, next) {
  return renderManageUsersIndex(req, res)
})
...
router.get('/edit-email-notifications', function (req, res) {
  normalizeUsers(req)
...
router.get('/edit-emailp-notifications', function (req, res) {
  ...
  return res.redirect(`edit-email-notifications${queryString}`)
})
...
router.get('/edit-appeal-stage', function (req, res) {
  normalizeUsers(req)
  ...
  res.render(req.baseUrl.replace(/^\//g, '') + '/edit-appeal-stage')
})
...
router.post('/email-notifications', function (req, res) {
  ...
  if (editUserId) {
    return res.redirect('edit-appeal-stage')
  }
...
router.post('/edit-detailed-email-notifications', function (req, res) {
  ...
  const editUserId = req.session.data['edit-user-id'] || req.session.data['manage-user-id'] || req.session.data['id']
  let targetUserId = editUserId || null
  if (editUserId) {
    const userIndex = req.session.data['users'].findIndex((entry) => entry.id === editUserId)
    if (userIndex >= 0) {
      req.session.data['users'][userIndex].appealTypes = appealTypes
  ...
  if (targetUserId) {
    req.session.data['edit-user-id'] = targetUserId
    req.session.data['manage-user-id'] = targetUserId
  }

  delete req.session.data['email-notifications']
  delete req.session.data['planning-type']

  if (!req.session.data['updated-email'] && targetUserId) {
    const updatedUser = req.session.data['users'].find((entry) => entry.id === targetUserId)
    if (updatedUser) {
      req.session.data['updated-email'] = updatedUser.email
    }
  }
  req.session.data['action'] = 'updated'
...
router.post('/appeal-stage', function (req, res) {
  ...
  const bodySelectedStages = req.body ? req.body['appeal-stage'] : undefined
  const selectedStages = bodySelectedStages !== undefined
    ? bodySelectedStages
    : req.session.data['appeal-stage']
  ...
  const appealStagesRaw = Array.isArray(selectedStages)
    ? selectedStages
    : (selectedStages ? [selectedStages] : [])
  const appealStages = appealStagesRaw.filter((stage) => stage && stage !== '_unchecked')
  ...
  return res.redirect(`${req.baseUrl}/index`)
})
