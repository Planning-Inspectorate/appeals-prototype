...
router.post('*', function(req, res, next){
  if (req.session.data['cya']) {
    const isOtherAppealsRoute = req.path.endsWith('/appeal-process/other-appeals')
    const isOtherAppealReferenceRoute = req.path.endsWith('/appeal-process/other-appeal-reference')

    // In CYA mode, allow this mini-flow to continue when adding more references
    if ((isOtherAppealsRoute && req.session.data['other-appeals'] == 'Yes') || isOtherAppealReferenceRoute) {
      return next()
    }
...
router.get('/consultation/other-parties-check', function (req, res, next) {
  if (!req.session.data['other-parties-check']) {
...
router.get('/po-report/policies-check', function (req, res, next) {
  if (!req.session.data['policies-check']) {
    req.session.data['policies-check'] = 'Yes'
  }
  next()
})

router.get('/po-report/supplementary-check', function (req, res, next) {
  if (!req.session.data['supplementary-check']) {
    req.session.data['supplementary-check'] = 'No'
  }
  next()
})
...
router.post('/appeal-process/other-appeal-reference', function (req, res) {
  const otherAppealReference = (req.session.data['other-appeal-reference'] || '').toString().trim()

  req.session.data['appeal-process-started'] = 'true'
  req.session.data['other-appeal-reference-complete'] = 'true'

  if (otherAppealReference) {
    if (!Array.isArray(req.session.data['other-appeal-references'])) {
      req.session.data['other-appeal-references'] = []
    }

    if (!req.session.data['other-appeal-references'].includes(otherAppealReference)) {
      req.session.data['other-appeal-references'].push(otherAppealReference)
    }
  }
